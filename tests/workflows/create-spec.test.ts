...
const WORKFLOW_NAME = 'create-spec';
const STEP_SLUGS = ['analyze-prd', 'draft-spec', 'validate-spec', 'approval'];
...
beforeEach(async () => {
  testDir = join(tmpdir(), `sinfonica-create-spec-test-${Date.now()}`);
...
async function scaffoldIndex(sessionId: string): Promise<void> {
  ...
  await createWorkflowIndex({
    cwd: testDir,
    sessionId,
    workflowId: `w-create-spec-${Date.now()}`,
    goal: 'Create a technical specification from an approved PRD',
    steps,
  });
}
...
describe('create-spec: loadWorkflowDef', () => {
...
describe('create-spec: sequential loading order', () => {
...
describe('create-spec: resume from mid-workflow', () => {
...
describe('create-spec: approval gate at final step', () => {
