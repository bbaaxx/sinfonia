# Technical Specification: Routed `create-spec` Orchestration for `sinfonia-maestro`

## 1) Scope and Constraints

### In scope
- Request payload normalization for routed execution to `@sinfonia-maestro`.
- Skill-context loading from `.opencode/skills/sinfonia-create-spec/SKILL.md`.
...
- Maintain explicit routing target: `sinfonia-maestro`.
...
- `target_persona`: must equal `sinfonia-maestro`.
- `skill_path`: must equal `.opencode/skills/sinfonia-create-spec/SKILL.md` and file must exist.
...
```json
{
  "address_to": "@sinfonia-maestro",
  "workflow": "create-spec",
  "skill_context_path": ".opencode/skills/sinfonia-create-spec/SKILL.md",
...
Validation rules:
- `address_to` is exactly `@sinfonia-maestro`.
...
- Dispatch envelope exists and targets `@sinfonia-amadeus`/assigned persona as planned.
- Task prompt exists and targets `sinfonia-maestro` with required response format.
