...
Sinfonica coordinates multi-agent workflows, but it must remain understandable, inspectable, and easy to operate inside a normal repository. A hosted control plane or remote orchestration service would increase operational complexity and blur ownership boundaries.

## Decision

Sinfonica is defined as a local-first orchestration framework.

- Runtime assets live in repository-managed paths (for example, `.sinfonica/` and `.opencode/`).
...
- Neutral: teams can still layer external systems around Sinfonica if needed.

## Alternatives Considered

1. Build Sinfonica primarily as a hosted SaaS orchestration platform.
