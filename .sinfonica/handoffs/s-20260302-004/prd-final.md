# PRD: Sinfonica Orchestration Framework (Retrospective Baseline)

## Problem Statement

AI-assisted software delivery often fails operationally due to inconsistent handoffs, loss of session context, and weak process guardrails. Sinfonica exists to provide a local-first orchestration framework that makes multi-agent workflows explicit, traceable, and enforceable through typed handoffs, durable state, and approval-driven progression.

## Product Scope (Current)

Sinfonica is a TypeScript/Node.js CLI framework that currently provides:

- Project bootstrap and runtime scaffolding via `sinfonica init`.
- Persona contract validation via `sinfonica validate`.
- Enforcement rule visibility via `sinfonica rules`.
- Built-in workflows (`create-prd`, `create-spec`, `dev-story`, `code-review`) with fixed persona routing.
- Session-level workflow state persistence in `.sinfonica/handoffs/<sessionId>/workflow.md`.
...
- Individual developers/operators bootstrapping or running Sinfonica in a local repository.
...
As a developer, I want to initialize Sinfonica in my repository so that multi-agent workflows and guardrails are available immediately.

**Acceptance Criteria**

- [ ] Running `sinfonica init -y` creates `.sinfonica/` runtime structure and required generated artifacts documented in CLI reference.
...
- [ ] Each session has a canonical `.sinfonica/handoffs/<sessionId>/workflow.md` with required frontmatter and sections.
...
- [ ] `sinfonica rules` lists registered rules and supports machine-readable output (`--json`).
