# Dispatch Envelope: Stage 01 -> @sinfonica-coda
...
- `.sinfonica/handoffs/s-20260302-002/return-05-rondo.md`
...
4. Include a small Maestro prompt correction for subagent routing clarity:
   - In `agents/maestro.md` Subagent table (`When to spawn`), update the `sinfonica-metronome` row to align with Metronome's core role as context-management specialist (context pressure/compaction/memory recovery), not primary QA/test planning.
   - Keep routing intent consistent with Maestro handoff instructions and routing table.
   - If a generated stub mirror exists (`.opencode/agent/sinfonica-maestro.md`), keep it aligned.
...
- [ ] Maestro prompt routing row for `sinfonica-metronome` reflects context-management trigger semantics
